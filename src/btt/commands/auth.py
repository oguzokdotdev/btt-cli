...
def auth_main(ctx: typer.Context):
    ...
    if not config_data:
        typer.secho("Configuration is not set yet. Please run 'btt config --help' first.", fg=typer.colors.RED)
        raise typer.Exit(code=1)
        
    api_id = config_data.get("api", {}).get("id")
    api_hash = config_data.get("api", {}).get("hash")

    if not api_id or not api_hash:
        typer.secho("Missing 'api_id' or 'api_hash' in configuration.", fg=typer.colors.RED)
        typer.secho("Please set them using: btt config api.id 12345678 and btt config api.hash abcdefg0", fg=typer.colors.YELLOW)
        raise typer.Exit(code=1)
